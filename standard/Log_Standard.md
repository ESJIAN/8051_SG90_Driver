...
好的，我将按照你提供的格式规范重新整理 [[部门仓库/软件部/开发规范/日志规范]] 这篇笔记。
1. 概要
1.1. 什么是日志？
...
2.2. 日志等级设置规范
日常开发中有四种比较常见的日志打印等级，不同的等级适合在不同的时机下打印日志。
主要使用的有以下四个等级：
- DEBUG
...
在编写核心业务逻辑代码时，如遇到?if...else...或者 switch?这样的条件，可以在分支的首行就打印日志，这样排查问题时，就可以通过日志，确定进入了哪个分支，代码逻辑更清晰，也更方便排查问题了。
建议：
public void doSth(){
